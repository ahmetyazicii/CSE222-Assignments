public class OfficeChairs implements Supply{

    private final int[][][] supply;
    private  final int modelNum = 7;
    ...
    public void print(int model, int color) {
        System.out.printf("Office Chairs Model %d Color %d = %d\n",model+1,color+1,getAmount(model,color));
    }
    ...
    public boolean areNeedSupply() {
        ...
        for (int i=0;i<modelNum;++i)
            for (int j=0;j<colorNum;++j)
                if(isNeedSupply(i,j)) {
                    System.out.printf("Office Chairs Model %d Color %s is need to be supplied.Current in store: %d\n",i+1,printColor(j),getAmount(i,j));
                    check = true;
                }
        return check;
    }
