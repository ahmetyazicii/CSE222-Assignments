public class BookCases implements Supply{
    ...
    public void print(int model, int color) {
        System.out.printf("Bookcases Model %d= %d\n",model+1,getAmount(model,color));
    }
    @Override
    ...
    public boolean areNeedSupply() {
        ...
        for (int i=0;i<modelNum;++i)
                if(isNeedSupply(i,0)){
                    System.out.printf("Book Cases Model %d is need to be supplied.Current in store: %d\n",i+1,getAmount(i,0));
                    check = true;
                }
        return check;
    }
